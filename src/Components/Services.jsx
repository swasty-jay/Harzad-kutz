...
function Services() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      {/* Tabs */}
      <div className="flex justify-center gap-2 md:gap-4 text-[10px] md:text-[12px] mb-8 flex-wrap">
        {tabs.map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`px-2 py-1 rounded-lg text-[10px] md:text-sm font-medium transition ${
              activeTab === tab
